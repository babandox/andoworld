...
def initialize_live_swarm(
    ...
    *,
    ...
    rng = Random(seed)
    particles = [
        Particle(state=_create_live_particle_state(country, macro_update, index=index, rng=rng), weight=1.0)
...
def run_live_engine(
    ...
    *,
    ...
    return LiveRunResult(day_summaries=summaries, readouts=readouts)
...
def _apply_live_observation_to_particle(state: LiveParticleState, observation: LiveDayObservation) -> None:
    ...
    if observation.event_count > 0 and observation.threat_lambda >= state.seed_threshold:
